...
const registerSubString = print(SUPPLIER_REGISTERED_SUBSCRIPTION);
const updateSubString = print(SUPPLIER_DETAILS_UPDATED_SUBSCRIPTION);
const deactivateSubString = print(SUPPLIER_DEACTIVATED_SUBSCRIPTION);

export const useSupplierSubscriptions = () => {
  const { upsertInfiniteData, updateInInfiniteData, removeFromInfiniteData } =
    useMutateCache<SupplierModel>();
  useEffect(() => {
    ...
    const subscribeRegister = async () => {
      try {
        for await (const result of registerIter) {
          ...
          if (data?.onSupplierRegistered) {
            upsertInfiniteData(
              data.onSupplierRegistered as SupplierModel,
              SUPPLIERS_QUERY_KEY(),
              'suppliers'
            );
          }
    ...
    const subscribeUpdate = async () => {
      try {
        for await (const result of updateIter) {
          const data = result.data;
          if (data?.onSupplierDetailsUpdated) {
            updateInInfiniteData(
              data.onSupplierDetailsUpdated as SupplierModel,
              SUPPLIERS_QUERY_KEY(),
              'suppliers'
            );
          }
    ...
    const subscribeDeactivate = async () => {
      try {
        for await (const result of deactivateIter) {
          const data = result.data;
          if (data?.onSupplierDeactivated) {
            const id = (data.onSupplierDeactivated as { id: string }).id;
